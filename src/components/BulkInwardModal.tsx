...
export function BulkInwardModal() {
    ...
    // FIX: Separate States
    const [receivedDate, setReceivedDate] = useState(new Date().toISOString().split('T')[0]) // Default Today
    const [documentDate, setDocumentDate] = useState("") // Bill Date (Empty initially)
    ...
    const handleProductAdded = (newProduct: Product) => { /* ... same ... */
    ...
    const handleSubmit = () => {
        if (!partyName || !documentNumber || !documentDate) { alert("Please fill Vendor, Doc Number & Bill Date"); return; }

        // ... validation same ...
        const itemsWithProduct = rows.filter(r => r.productId);
        const validItems = itemsWithProduct.filter(r => r.quantity > 0);
        if (validItems.length === 0) { alert("No valid items"); return; }

        addTransaction({
            id: crypto.randomUUID(),
            date: receivedDate,         // <-- Physical Date
            documentDate: documentDate, // <-- Bill Date
            type: 'IN',
            documentType, documentNumber, partyName,
        ...
        <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                    <Button className="bg-emerald-600 hover:bg-emerald-700 text-white gap-2 shadow-sm"><ArrowDownCircle className="h-4 w-4" /> Stock In (Purchase)</Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[900px] bg-white text-slate-900 max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid gap-6 py-4">
                        <div className="grid grid-cols-4 gap-4 bg-white p-4 rounded-md border border-slate-200 shadow-sm">
                            ...
                            <div className="col-span-1">
                                <Label className="text-emerald-700">Received Date</Label>
                                <Input type="date" value={receivedDate} onChange={e => setReceivedDate(e.target.value)} />
                            </div>
                        </div>

                        <TransactionItemTable rows={rows} setRows={setRows} products={products} type="IN" onAddNewProduct={(name, index) => { setPendingName(name); setActiveRowIndex(index); setIsCatalogOpen(true); }} />
                    </div>
                    <DialogFooter>
